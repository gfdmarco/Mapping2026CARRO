...
STEERING_TAU = 0.15      # s   -> filtro passa-baixa do comando de steering
...
class VehicleController:
    """Junta o controle lateral (MPC com fallback pure pursuit) e
    longitudinal (PID).

    O MPC antecipa as curvas pelo horizonte e rejeita o ruido da
    percepcao; quando o caminho e curto/degenerado demais para montar o
    horizonte, o pure pursuit assume no mesmo ciclo.
    ...
    def __init__(self, target_speed=TARGET_SPEED,
        ...
        self.mpc = ModelPredictiveController() if use_mpc else None
        self.mpc_engaged = False
        self.longitudinal = PID(KP, KI, KD)
        self.last_steering = 0.0

    def compute(self, path, speed, dt, target_speed=None, use_mpc=True):
        # target_speed por ciclo: usado no modo TRACK, em que o perfil de
        # velocidade da trajetoria global varia ponto a ponto
        if target_speed is not None:
            self.target_speed = target_speed

        # use_mpc=False forca o pure pursuit (volta 1: caminho local
        # ruidoso da percepcao, comportamento identico ao pipeline
        # original); o MPC entra na trajetoria global, que e suave
        steering = None
        if self.mpc is not None and use_mpc:
            if not self.mpc_engaged:
                # transferencia sem solavanco: o MPC parte do steering
                # que o pure pursuit estava aplicando, nao de zero
                self.mpc.last_delta = self.last_steering * self.mpc.max_steer_rad
            steering = self.mpc.computeSteering(path, speed, dt)
        self.mpc_engaged = use_mpc and steering is not None
        if steering is None:
