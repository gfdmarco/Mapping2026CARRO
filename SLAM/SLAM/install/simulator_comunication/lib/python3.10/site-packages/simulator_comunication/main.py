...
class fsdsNode(Node):
    ...
    def odometry_process(self):
        ...
        odom.child_frame_id = 'base_link' # Padronizado para convenção oficial do ROS
        ...
        # Simulando a gravidade real no eixo Z para o Cartographer
        imu.linear_acceleration.z = float(-kinematics.linear_acceleration.z_val) + 9.81

        imu.orientation = odom.pose.pose.orientation

        self.imu_publisher.publish(imu)

        # --- 3. PUBLICAÇÃO DA ÁRVORE DE TF (odom -> base_link) ---
        t = TransformStamped()
        t.header.stamp = tempo_atual
    ...
    def perception_process(self):
        try:
            ...
            msg_cones.header.frame_id = 'base_link' 
            ...
            for idx, cone in enumerate(cones):
                ...
                landmark.translation_weight = 15.0 
    ...
    def publish_pc2(self, points, timestamp):
        ...
        msg.header.frame_id = 'lidar_link' # Nome limpo e padronizado para o ROS 2
        ...
        ros_points[:, 2] = points[:, 2]   # Z_ros apontando corretamente para cima
